...
class Woocommerce_Doordash_Pickup_Location {
	...
	public function get_delivery_days() {
		...
		$number_of_days = apply_filters( 'wcdd_delivery_number_of_days_ahead', 14 );
	...
	public function get_delivery_times_for_date( $datestamp ) {
		...
		// Get the lead time for the shop
		...
		foreach( $ranges as $range ) {
			...
			$range_values = $ddhours->fill_range( $range[0] + $lead_time, $range[1], $datestamp );
	...
	public function get_next_valid_time() {
		...
		$today = strtotime( 'today' ) - $gmt_offset; // 12:00am LOCAL
		$current_day = $today;
		$ddhours = new Woocommerce_Doordash_Hours();
		$number_of_days = apply_filters( 'wcdd_delivery_number_of_days_ahead', 14 );
		$lead_time = intval( get_option( 'woocommerce_doordash_lead_time' ) ) * MINUTE_IN_SECONDS;

		// Set target delivery time to current time + lead time
		$time = time() + $lead_time;
		...
		while ( $i  <= $number_of_days ) {
			...
			if ( ! empty( $day_hours ) ) {
				...
				foreach( $ranges as $range ) {
					...
					if ( $time >= $open && $time <= $close ) return $time;

					// If the target time is less than the open and close time, we can delivery at the open time (plus lead time)
					if ( $time <= $open && $time <= $close ) return $open + $lead_time;
				}
