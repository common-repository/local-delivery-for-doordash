...
class Woocommerce_Doordash_Shipping_Method extends WC_Shipping_Method {
	...
	public function calculate_shipping( $package = array() ) {
		...
		// Handle tips
		$tip = WC()->session->get( 'doordash_tip_amount' );
		if ( $tip > 0 ) {
			// Only add the fee if there is a tip attached
			WC()->cart->add_fee( __( 'Dasher Tip', 'local-delivery-by-doordash' ), $tip );
		}
		...
		$this->add_rate( array(
			'label' => $this->title,
			'package' => $package,
			'cost' => $delivery->get_fee(), // + doubleval( $tip ), // $doordash_pickup_location,//$this->cost,
			'meta_data' => array(
		...
		// if ( empty( $delivery->get_fee() ) ) WC()->session->set( 'doordash_external_delivery_id', '' );
